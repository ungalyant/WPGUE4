	...
	else
	{
		...
		if (CurrentRequestId == MoveId)
		{
			...
			if (!bIsUsingMetaPath && Path.IsValid() && Path->IsValid())
			{
				...
				//Last step logic, added by white paper 08-09-2016
				//*****************************************
				bool foundLastStepInfo = false;
				LastStepSqDistFromIndex = LastStepDistance * LastStepDistance;
				LastStepIndex = Path->GetPathPoints().Num() - 1;
				bShouldTakeLastStep = true;
				bLastStepTaken = false;
				
				FVector prevSegStart;
				FVector segStart;
				FVector segEnd;
				FVector inDir;
				FVector outDir;

				while (!foundLastStepInfo)
				{
					segStart = *Path->GetPathPointLocation(LastStepIndex - 1);
					segEnd = *Path->GetPathPointLocation(LastStepIndex);

					float distSq = (segStart - segEnd).SizeSquared();

					if (distSq < LastStepSqDistFromIndex)
					{
						if (Path->GetPathPoints().IsValidIndex(LastStepIndex - 2))
						{
							prevSegStart = *Path->GetPathPointLocation(LastStepIndex - 2);
							LastStepSqDistFromIndex -= distSq;

							inDir = segStart - prevSegStart;
							outDir = segEnd - segStart;
							inDir.Normalize();
							outDir.Normalize();

							if (AcceptableLastStepTurn < FMath::Acos(FVector::DotProduct(inDir, outDir)))
							{
								LastStepIndex--;
								continue;
							}
						...
						bShouldTakeLastStep = false;
						break;

					}
					else
					{
						foundLastStepInfo = true;
					}
				}
				//****************************************
			}
...
void UPathFollowingComponent::SetMovementComponent(UNavMovementComponent* MoveComp)
{
	...
	if (MoveComp)
	{
		...
		LastStepDistance = NavAgentProps.LastStepDistance;
		AcceptableLastStepTurn = NavAgentProps.AcceptableLastStepTurn;
...
void UPathFollowingComponent::SetMoveSegment(int32 SegmentStartIndex)
{
	...
	if (Path.IsValid() && Path->GetPathPoints().IsValidIndex(SegmentStartIndex) && Path->GetPathPoints().IsValidIndex(EndSegmentIndex))
	{
		...
		{
			NextDestination = Path->GetPathPointLocation(MoveSegmentEndIndex + 1);
	...
	if (bCanUpdateState && Status == EPathFollowingStatus::Moving)
	{
		...
		else if (bFollowingLastSegment)
		{
			...
			if (DestinationActor.IsValid() && Path->IsPartial() == false)
			{
				...
				NextDestination.Set(NULL, GoalLocation);
...
void UPathFollowingComponent::FollowPathSegment(float DeltaTime)
{
	...
	if (bAccelerationBased)
	{
		...
		if (MoveSegmentEndIndex == LastStepIndex && !bLastStepTaken)
		{
			float sqDistFromEnd = (CurrentTarget - CurrentLocation).SizeSquared();

			if (sqDistFromEnd < LastStepSqDistFromIndex)
			{
				bLastStepTaken = true;
				OnLastStep.Broadcast();
			}
		}
	}
	else
...
FVector UPathFollowingComponent::GetNextDirection() const
{
	if (CurrentDestination.Base)
	{
		...
		{
			// calculate direction to based destination
			const FVector SegmentStartLocation = *CurrentDestination;
			const FVector SegmentEndLocation = *NextDestination;
