...
class AIMODULE_API UPathFollowingComponent : public UActorComponent, public IAIResourceInterface
{
	...
	DECLARE_MULTICAST_DELEGATE(FLastStepSignature);
	...
	*  @param RequestID - request to resume, FAIRequestID::CurrentRequest means restor current request, regardless of its ID */
	...
	FORCEINLINE void SetNotifyFromEndDistance(float InLastStepDistance) { LastStepDistance = InLastStepDistance; }
	...
	FORCEINLINE FVector GetNextTargetLocation() const { return *NextDestination; }
	FORCEINLINE FBasedPosition GetCurrentTargetLocationBased() const { return CurrentDestination; }
	FORCEINLINE FBasedPosition GetNextTargetLocationBased() const { return NextDestination; }
	...
	FVector GetNextDirection() const;
...
protected:
	...
	/**Centimetres leading up to the end of the path used to fire an event for an AI playing a "Last Step" animation*/
	float LastStepDistance;

	/**Centimetres leading up to the end of the path used to fire an event for an AI playing a "Last Step" animation*/
	float AcceptableLastStepTurn;

	/** If the length of LastStepDistance is longer than the last segment, what segment does the event fire at? */
	float LastStepIndex;

	/** How many square centimetres from the end of the last step segment does the event fire (purely for cheaper comparisons) */
	float LastStepSqDistFromIndex;
	...
	/** destination for current path segment */
	FBasedPosition NextDestination;
	...
	/** Whether the LastStep event has already fired whilst on this path*/
	uint32 bLastStepTaken : 1;

	/** If the character exceeds acceptable last step angle, this will be set to false */
	uint32 bShouldTakeLastStep : 1;
