...
struct ENGINE_API FNavAgentProperties : public FMovementProperties
{
	...
	/** The Distance from the end of any movement at which the last step event is called*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = MovementProperties)
		float LastStepDistance;

	/** If the last step event would be fired before a turn in the nav path, what (if any) is an acceptable
		angle to turn at if the angle is over this (or the value is -1) the character will not turn.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = MovementProperties)
